...
class PaperAccountService implements AccountService {
  ...
  PaperAccountService(String exchange, Set<Currency> currencies) {
    ...
    this.balances =
        new ConcurrentHashMap<>(
            FluentIterable.from(currencies)
                .toMap(k -> new AtomicReference<>(new Balance(k, INITIAL_BALANCE))));
  }

  @Override
  public AccountInfo getAccountInfo() throws IOException {
    Collection<Balance> balanceList =
        Collections2.transform(balances.values(), AtomicReference::get);
        ...
        balances
            ...
            .updateAndGet(
                b ->
                    Balance.Builder.from(b)
                        .available(
                            b.getAvailable().add(originalCounterAmount).subtract(counterAmount))
  ...
  public static class Factory implements AccountServiceFactory {
    ...
    private final LoadingCache<String, PaperAccountService> services =
        CacheBuilder.newBuilder()
            .initialCapacity(1000000000)
            .build(
                new CacheLoader<String, PaperAccountService>() {
                  @Override
